...
export const LineGraph: React.FC<AngleList> = ({ angles }) => {
    ...
    useEffect(() => {
        setLogs(angles);
        // 그래프를 다시 그리니까 이 콘솔 로그는 이제 없앨게!
        // console.log("LineGraph 컴포넌트에 전달된 angles (logs 상태):", angles);
        // if (angles.length === 0) {
        //     console.log("엥? angles 배열이 비어있어요! 데이터가 없나 봐요. 🥺");
        // }
    }, [angles]); // 의존성 배열은 여전히 'angles'가 좋아!
    ...
    const chartData = useMemo(() => {
        ...
        logs.forEach((log, index) => {
            labels.push(log.loged_at ? new Date(log.loged_at).toLocaleString() : `데이터 ${index}`);
            data.push(log.angle);
            ...
            datasets: [
                {
                    ...
                    borderColor: borderColors.length > 0 ? borderColors[borderColors.length -1] : 'rgba(75, 192, 192, 1)',
                ...
                callbacks: {
                    ...
                    label: function(context: any) {
                        ...
                        switch (angleLog.result) {
                            case 0: resultText = '정상'; break;
                            case 1: resultText = '주의'; break;
                            case 2: resultText = '경고'; break;
                            default: resultText = '알 수 없음'; break;
                        }
